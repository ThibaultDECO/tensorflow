...
absl::Status TryDumpSdcLog(se::Stream* stream, void* log_ptr,
                           const HloModule& hlo_module,
                           const DebugOptions& debug_options) {
  se::DeviceMemory<uint8_t> device_log_ptr(
      se::DeviceMemoryBase(log_ptr, kLogSizeBytes));
  se::cuda::SdcLog sdc_log =
      se::cuda::SdcLog::FromDeviceMemoryUnchecked(device_log_ptr);
...
void DumpSdcLog(se::Stream* stream, void* log_ptr,
                XlaCustomCallStatus* call_status, const HloModule& hlo_module,
                const DebugOptions& debug_options) {
  absl::Status status =
      TryDumpSdcLog(stream, log_ptr, hlo_module, debug_options);
  if (!status.ok()) {
    LOG(ERROR) << "[SDC LOG] Failed to dump SDC log: " << status;
    XlaCustomCallStatusSetFailure(call_status, status.message().data(),
                                  status.message().size());
  }
}
...
absl::StatusOr<bool> ThunkChecksumTracingPass::Run(
  ...
  auto dump_sdc_log = [=](se::Stream* stream, void** data, const char*, size_t,
                          XlaCustomCallStatus* call_status) {
    DumpSdcLog(stream, data[0], call_status, *hlo_module_ptr, debug_options);
  };
  ...
  TF_ASSIGN_OR_RETURN(auto sdc_dump_thunk,
                      CustomCallThunk::Create(
                          Thunk::ThunkInfo(), "xla_gpu_sdc_log_dump",
                          std::move(dump_sdc_log), {shaped_log_slice}, {}, ""));
