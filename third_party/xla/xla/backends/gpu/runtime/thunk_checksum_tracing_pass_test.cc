...
TEST(ThunkChecksumTracingPassTest, IsNoOpWhenSdcTracingDisabled) {
  ...
  EXPECT_THAT(root_thunk->thunks(), SizeIs(1));
  EXPECT_EQ(root_thunk->thunks()[0].get(), fake_thunk_ptr);
}
...
TEST(ThunkChecksumTracingPassTest, InsertsSdcThunks) {
  ...
  EXPECT_EQ(sub_thunks[1].get(), fake_thunk_ptr);
